...
class SudokuPuzzle(Puzzle):
    ...
    # TODO (Task 1): COMPLETE THE METHOD BELOW / override fail_fast
    ...
    def fail_fast(self):
        ...
        for r in range(len(self._symbol_set)):
            for c in range(len(self._symbol_set)):
                if self._symbols[r][c] == '*':
                    union_set = self._row_set(r).union(self._column_set(c), self._subsquare_set(r, c))
                    union_set.remove('*')
                    if union_set == self._symbol_set:
                        return True
        return False
